...
Within Uganda specifically, there are over a million South Sudanese refugees that are crossing the border and living in Ugandan refugee camps, around 80% of these are women and children. With so many children coming into these camps, education is very difficult. It is not uncommon to see classrooms filled with multiple ages, grades and 200 students for every one teacher. My original hope for this project was to include data from these refugee camps, and compare it to the districts that do not have refugees. 
 ...
 26% of the population live in urban settings and 74% live in rural settings. 



Knowing all of this, I was very interested in seeing if I could find some of the data myself and get similar results. I also wanted to see if there was a connection to food price and education. The rest of teh post will walk you through my steps and teh problems I encountered. 
...
I was able to find four years of data (2013-2016) that looks at the total number of Ugandan children, split by gender, who attend grades Primary 1 through Primary 7 respectively. Primary 1 is the same as Kindergarten, Primary 2 as first grade, and so on. Then I found a data set that had the Gross Enrollment Rate (GER), and Net Enrollment Rate for Secondary school, separated by district. 
...
The second main trend we can see here is that the Northen region is much lower than 



### Now we can start to look at some models

First I wanted to know how strong the visual trend of school enrollment by district was. To do this 
...
Now i wanted to see if year has an effect, as we woudl hope that as time goes on, the enrollment increases 
(CAN THIS BE ONE MODEL)
...
Then we do it again for the NER just to be safe 

```{r}
 lm(NER ~ region, data = ug_secondaryNER)  %>%
  summary() %>% 
  xtable() %>% 
  kable(caption = "Think Of a Captaion", digits = 4) %>% 
  kable_styling(latex_options = "HOLD_position")
...
```{r}
lm(NER ~ Years, data = ug_secondaryNER) %>% 
  summary()
```


NOw we cna look at our primary school data
...
males are less likey to be enrolled, there is a significant difference in the enromelnt between grade levels 
